...
**Процесс сборки:**
1. Откройте файл решения `ZZZ-PS-Launcher.slnx` двойным кликом мыши.
2. В верхнем меню Visual Studio выберите **Сборка** > **Собрать решение** (Build > Build Solution).

Готовый `.exe` файл будет находиться по пути:
...
3. Заполните данные:
   ...
   * **Коммит сервера:** Выберите нужную версию из выпадающего списка (содержит рекомендованные и последние актуальные коммиты для быстрого доступа).
