...
## ⚙️ Совместимость
> [!IMPORTANT]
> На данный момент лаунчер оптимизирован под версию **yoshunko**, и **remielle**. 
> Поддержка ранних версий (Yidhari-ZS, Orphie-zs и др.) не гарантируется.   
> Не смотря на то, что сервер **remielle** работает на windows, ланучеру всё ещё требуется wsl  для управления git.   
>Данная проблема будет решена в будущем.

## 📋 Требования к системе
Перед использованием убедитесь, что у вас настроены:
* **WSL** (Windows Subsystem for Linux) с установленным дистрибутивом.
* **ZVM** (Zig Version Manager), установленный внутри вашего WSL-дистрибутива (для **yoshunko**), или внутри вашей windows системы (для **remielle**).
* **Рабочий сервер**, развернутый в WSL-дистрибутиве (для **yoshunko**) или в windows системе (для **remielle**).
...
Нажмите на ссылку справа от параметра **Целевое расположение**, чтобы открыть папку с лаунчером или перейдите по пути `\bin\Release\net10.0-windows\publish\win-x64`
...
3. Заполните данные:
   ...
   * **Тип сервера:** Выберите сервер который вы используете (**yoshunko**, или **remielle**)
